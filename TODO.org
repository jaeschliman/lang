...
* TODO clean up (new BCBuilder(vm)) callsites (ensure delete)
...
* TODO more thread manipulation functions
  (current-thread)
  (list-all-threads)
  (thread-status thread)
  (kill-thread thread)
* TODO semaphore, mutex
* TODO use bitmask for 'primitive' primOps (aka @send apply etc)
