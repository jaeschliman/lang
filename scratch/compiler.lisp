...
(define (walk-exprs es walk)
  (dolist (e es) (walk-form e walk)))
...
(define (walk-letrec e walk)
  (let ((binds (cadr e))
    ...
    (with-expression-context (body)
      ...
      (walk-exprs body walk))))

(define (walk-let e walk)
  (let ((binds (cadr e))
        (body (cddr e)))
    (cond ((nil? binds) (walk-exprs body walk))
          (#t (dolist (pair binds) (walk-form (cadr pair) walk))
              (with-expression-context (body)
...
(define (walk-lambda e walk in-call-position)
  (let* ((args (caddr e))
    ...
    (cond
      ((and inline? (nil? args)) (walk-exprs body walk))
         ...
         (let ((was-in-call-position *in-call-position*))
           (binding ((*in-call-position* #f))
             (case (car e)
               ...
               (with-special-binding (walk-exprs (cddr e) walk))
               (#t (binding ((*in-call-position* #t)) (walk-form (car e) walk))
                   (walk-exprs (cdr e) walk))))))))
