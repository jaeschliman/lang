...
* IN-PROGRESS convert meta reader state to a class for better performance
  basically want a way to keep from allocating a hash table for every call to `any`
* IN-PROGRESS looks like there's an occasional bug somewhere in ht_grow
stack trace:
...
* TODO per-thread stacks (maybe just value stacks?) -- can use xarray if value only
...
* TODO some error when spawning a fresh thread occasionally
stack trace:
...
* TODO have fork-with-priority take a thunk rather than a snapshot
