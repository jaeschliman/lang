...
* TODO asm: proper names and sourcelocations for %nlambda
...
* IN-PROGRESS asm: inline lambdas where possible
...
* TODO rework the definitions in boot/0 to be amenable to TL jump opts
  ...
  not `set`
* TODO rewrite meta `seq` to use if failure?
...
* TODO asm: eliminate closures from letrec where possible
