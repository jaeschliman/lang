...
* IN-PROGRESS some kind of simple game -- maybe a remake of the alien abduction from xmas
...
* TODO blit with tint / blend modes
...
* IN-PROGRESS asm: optimize self tail calls into banging on args and a jump
...
* IN-PROGRESS clean up the compiler swap mess
...
* TODO load compiler in its own package
* TODO emacs indentation support
...
* TODO rework the definitions in boot/0 to be amenable to TL jump opts
  right now we special-case forms with `set-symbol-value` in the call position,
  not `set`
* TODO rewrite meta `seq` to use `if` for failure?
* TODO asm: we are probably creating more closures than needed.
...
* IN-PROGRESS asm: eliminate closures from letrec where possible
...
* TODO the original `bouncers.lisp` is now growing stack with N bouncers, it seems
* TODO exceptions STILL not propagating 'properly' from compiler
  execution halts (good) nothing is reported to console (bad)
* TODO rewrite `append` to not use up stack space
