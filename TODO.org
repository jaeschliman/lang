...
* TODO convert meta reader state to a class for better performance
  basically want a way to keep from allocating a hash table for every call to `any`
* TODO looks like there's an occasional bug somewhere in ht_grow
stack trace:
...
GOT conscar0
* TODO throw errors on failure in match-map, fix stx errors in boot files
...
* TODO move StandardObject ivars into a slot-vector
...
* TODO save slot names when defining classes
...
* TODO clean up the meta parser
  - get rid of the hash tables/recursion guards (don't really need them...)
