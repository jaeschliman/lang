    ...
    (when (>i sc 50)
      (let* ((delx (abs-maxf 10.0 (-f mx nx))
        ...
        (unless (or (eq 0.0 mag) ;(>f mag magmax)
          ...
          (let* ((scale (*f (sinf (-f 1.0 (/f (minf mag (*f magmax 0.995)) magmax))) 15.0 ))
                 (normdx (*f scale (/f delx mag)))
                 (normdy (*f scale (/f dely mag))))
            (set! dx (+f (*f 0.2 normdx) (lerpf 0.90 normdx dx)))
            (set! dy (+f (*f 0.2 normdy) (lerpf 0.90 normdy dy)))
...
(define (draw-one-box box)
  (let* (
         ...
         (x (aget box 0))
         (y (aget box 1))
         (back-x (f->i (+f (aget box 0) (*f -2.0 (aget box 2)))))
         (back-y (f->i (+f (aget box 1) (*f -2.0 (aget box 3)))))
    ...
    (#/lang/blit-from-screen back-buffer
                      ...
                      ;; back (point+ back 4@4)
                      ;; back (point+ back (make-point  (f->i (absf (aget box 2)))
                      ;;                                (f->i (absf (aget box 3)))))
                      1.0 1.0
                      ;1.02 13.3
                      )
