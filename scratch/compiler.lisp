...
(define (mark-inline-lambda e)
    (let* ((args (caddr e))
           (body (cdddr e)))
      (with-expression-context (body)
        (ctx-annot-put 'inline #t))
      (cond ((nil? args)
             ;; (print `(marking empty-call lambda: ,e))
             (mark-expressions body))
            (#t
             ;; (print `(marking inline-call lambda: ,e))
             (with-expression-context (body)
               (ctx-annot-put 'type 'let)
               (let ((idx 0))
                 (dolist (arg (ensure-list args))
       ...
       (let ((was-in-call-position *in-call-position*))
         (binding ((*in-call-position* #f))
           (case (car e)
             ...
             (#/lang/%let (mark-let e))
             (#/lang/%nlambda  (if was-in-call-position (mark-inline-lambda e) (mark-lambda e)))
...
(define (emit-inline-lambda-call it env)
    (let* ((lambda (car it))
           ...
           (vals (cdr it))
           (binds (zip args vals)))
      ;; (print `(emitting inline lambda: ,lambda))
